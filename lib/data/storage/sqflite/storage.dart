...
class SecureStorageSQflite {
  ...
  Future<bool> setRecord(PrefName key, String val) async {
    ...
    WriteLogs.storageSQflite('setRecord key: ${key.name} [${DateTime.now().millisecondsSinceEpoch - st}ms]');
    return (res != 0);
  }

  Future<String?> getRecord(PrefName key) async {
    var st = DateTime.now().millisecondsSinceEpoch;
    List<Map<String, Object?>>? res = await _db?.query('Params', where: 'key = ?', whereArgs: [key.name]);

    WriteLogs.storageSQflite('getRecord key: ${key.name} [${DateTime.now().millisecondsSinceEpoch - st}ms]');
    if ((res?.length ?? 0) > 0) {
  ...
  Future<int> delRecord(PrefName key) async {
    ...
    WriteLogs.storageSQflite('delRecord key: ${key.name} [${DateTime.now().millisecondsSinceEpoch - st}ms]');
    return res;
  }
  ...
  Future<bool> setHTTPcache(String key, String val) async {
    ...
    WriteLogs.storageSQflite('setHTTPcache key: $key [${DateTime.now().millisecondsSinceEpoch - st}ms]');
    return (res != 0);
  }

  Future<String?> getHTTPcache(String key) async {
    var st = DateTime.now().millisecondsSinceEpoch;
    List<Map<String, Object?>>? res = await _db?.query('OffLineData', where: 'key = ?', whereArgs: [key]);

    WriteLogs.storageSQflite('getHTTPcache key: $key [${DateTime.now().millisecondsSinceEpoch - st}ms]');
    if ((res?.length ?? 0) > 0) {
      return res?.first['value'].toString();
    } else {
      return null;
